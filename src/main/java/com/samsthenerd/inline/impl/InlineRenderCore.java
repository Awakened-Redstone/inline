...
public class InlineRenderCore {
    ...
    public static boolean textDrawerAcceptHandler(int index, Style style, int codepoint, RenderArgs args) {
        ...
        VertexConsumerProvider.Immediate immToUse = args.provider() instanceof VertexConsumerProvider.Immediate imm ? imm : VCPImmediateButImLyingAboutIt.of(args.provider());
        immToUse.draw();
...
//        matrices.multiply(args.matrix().getUnnormalizedRotation(new Quaternionf()).normalize());
        ...
        double maxSizeMod = InlineClientConfig.chatScaleCap;
