...
public interface RegexMatcher extends ContinuousMatcher {
     ...
     */
    class Standard implements RegexMatcher {
         ...
         * @param info
         */
        // TODO: Change the regex to a general one and use a registry or map to get the right matcher
        public Standard(String innerRegex, Identifier id, Function<String, InlineMatch> matcher, MatcherInfo info) {
            this.regex = Pattern.compile("(\\\\)?\\[" + "(?i)" + id.getPath() + SEPARATORS_REGEX + "(" + innerRegex + ")]");
            this.id = id;
            this.info = info;
            this.matcher = matcher;
        }
    ...
    class ChatStandard extends Standard {
        ...
        public ChatStandard(String innerRegex, Identifier id, BiFunction<String, MatchContext.ChatMatchContext, InlineMatch> matcher, MatcherInfo info) {
            super(innerRegex, id, (nop) -> null, info);
